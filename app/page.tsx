...
import { loadPacking, loadPairs, savePacking, savePairs } from "@/lib/storage";
import { HeaderRow, LinesRow, PairRow, PackingRow, ParsedDoc } from "@/lib/types";
...
export default function HomePage() {
  ...
  useEffect(() => { setPacking(loadPacking()); setPairs(loadPairs()); }, []);
  useEffect(() => { savePacking(packing); }, [packing]);
  useEffect(() => { savePairs(pairs); }, [pairs]);

  useEffect(() => {
    if (!docs.length) return;
    setHeaders(docsToHeaderRows(docs));
    setLines(buildLinesByAsn(docs, packing));
  }, [docs, packing]);
  ...
  return (
    <main className="app-shell">
      <div className="mb-6 text-center">
        <h1 className="text-5xl font-black tracking-tight text-brand-navy">ASN TOOL GM</h1>
      </div>
          ...
          <div className="card p-8">
            ...
            <div className="table-wrap">
              <table className="ui-table">
                ...
                <tbody>
                  {headers.map((row, i) => (
                    <tr key={i}>{["ASN No","ETA","ETD","Sold To","Bill To","Ship To","Location","Line No"].map((k) => <td key={k}>{(row as any)[k]}</td>)}</tr>
                  ))}
                </tbody>
              </table>
            </div>
      ...
      {tab === "packing" && (
        <section className="mt-8 space-y-8">
          <div className="card p-8">
            ...
            <div className="table-wrap">
              <table className="ui-table">
                ...
                <tbody>
                  {packing.filter((r) => !search || `${r.item} ${r.rev} ${r.pack}`.toLowerCase().includes(search.toLowerCase())).map((row, idx) => (
                    <tr key={idx}>
          ...
          <div className="card p-8">
            <h2 className="mb-5 text-3xl font-bold text-brand-navy">Pair Items</h2>
            <div className="table-wrap">
              <table className="ui-table">
                <thead><tr><th>Item A</th><th>Rev A</th><th>Item B</th><th>Rev B</th><th>Pair Code</th></tr></thead>
                <tbody>
                  {pairs.map((row, idx) => (
                    <tr key={idx}>
                      {["item_a","rev_a","item_b","rev_b","pair_code"].map((key) => (
                        <td key={key}><input className="w-full rounded-xl border border-brand-line px-3 py-2" value={String((row as any)[key] || "")} onChange={(e) => { const next = [...pairs]; next[idx] = { ...next[idx], [key]: e.target.value }; setPairs(next); }} /></td>
                      ))}
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <button className="small-btn" onClick={() => setPairs([...pairs, { item_a: "", rev_a: "", item_b: "", rev_b: "", pair_code: "" }])}>Add Row</button>
            </div>
          </div>
        </section>
      )}
          ...
          <div className="card p-8">
            ...
            <div className="table-wrap">
              <table className="ui-table">
                ...
                <tbody>
                  {lines.map((row, i) => (
                    <tr key={i}>{["ASN","Item","Rev","Quantity","Packing","Thùng chẵn","SL lẻ PCS","Tổng Cartons","Line No","Location","Packing Found","Calc Status"].map((k) => <td key={k}>{(row as any)[k]}</td>)}</tr>
                  ))}
                </tbody>
              </table>
            </div>
          ...
          <div className="card p-8">
            ...
            <div className="table-wrap">
              <table className="ui-table">
                ...
                <tbody>
                  {summary.map((row, i) => (
                    <tr key={i}><td>{row.Location}</td><td>{row["Thùng chẵn"]}</td><td>{row["Tổng số thùng lẻ"]}</td><td>{row["Tổng"]}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
